...
@section('content')
    <p class="py-4"><a href="{{ route('admin.shop.categories.create') }}" class="btn btn-success">Добавить категорию</a></p>
    <table class="table table-bordered table-striped">
        ...
        <tbody>
            @foreach($categories as $category)
                <tr>
                    <td><a href="{{ route('admin.shop.categories.show', $category) }}">{{ html_entity_decode(str_repeat('&mdash;', (int)$category->depth)) }}{{ $category->title ?: $category->name }}</a></td>
                    <td>{{ $category->slug }}</td>
                    <td>
                        <form method="POST"  class="list-inline-item"
                              action="{{ route('admin.shop.categories.first', $category) }}"
                        ...
                        <form method="POST" class="list-inline-item"
                              action="{{ route('admin.shop.categories.up', $category) }}"
                              data-bs-toggle="tooltip" data-bs-placement="bottom"
                        ...
                        <form method="POST" class="list-inline-item"
                              action="{{ route('admin.shop.categories.down', $category) }}"
                              data-bs-toggle="tooltip" data-bs-placement="bottom"
                        ...
                        <form method="POST" class="list-inline-item me-5"
                              action="{{ route('admin.shop.categories.last', $category) }}"
                              data-bs-toggle="tooltip" data-bs-placement="bottom"
                        ...
                        <a href="{{ route('admin.shop.categories.edit', $category) }}" class="list-inline-item"
